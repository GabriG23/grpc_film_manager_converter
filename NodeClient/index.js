...
// // New API for reading service parameters negotiated by the Node client with the server
app.get('/api/registration', isLoggedIn, userController.serviceRegistration);     // Registration
app.get('/api/readparameters', isLoggedIn, userController.readConversionParameters);  // Read negotiated parameters

// API to modify with image type and size dimensions
app.get('/api/films/public/:filmId/images/:imageId', isLoggedIn, imageController.getSingleImage);       // Get single image
app.post('/api/films/public/:filmId/images', isLoggedIn, storage.uploadImg, imageController.addImage);  // Upload a single image
